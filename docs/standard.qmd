...
#| fig-cap: "Note: Alternative of the above table."
value_labels_fig5 <- c("1" = "Never", "2" = "Infrequently", "3" = "Sometimes", "4" = "Frequently")

selected_data_numeric_fig5 <- df_numeric %>%  
    dplyr::select(dplyr::all_of(frq_qr_eng))

if (ncol(selected_data_numeric_fig5) > 0 && nrow(selected_data_numeric_fig5) > 0) {
  
  valid_cols_indices_fig5 <- vapply(selected_data_numeric_fig5, function(col) {
    is.numeric(col) && sum(!is.na(col)) > 0 
  }, logical(1))
  
  data_to_plot_fig5 <- selected_data_numeric_fig5[, valid_cols_indices_fig5, drop = FALSE]

  if (ncol(data_to_plot_fig5) > 0) {
    graph <- sjPlot::plot_stackfrq(
      data_to_plot_fig5,
      coord.flip = TRUE, 
      geom.colors = "Blues", 
      show.total = FALSE,
      legend.labels = value_labels_fig5,
      axis.labels = unlist(shortlabels[names(data_to_plot_fig5)])
    )
...
graph <- df_numeric %>%  
  select(all_of(c(frq_qr_eng, qr_impr_conf, qr_core_likert))) %>%  
  plot_stackfrq(
    coord.flip = TRUE, 
    geom.colors = "Blues", 
    show.total = FALSE,
    legend.labels = value_labels,
    axis.labels = c(shortlabels[frq_qr_eng], 
                   shortlabels[qr_impr_conf], 
                   shortlabels[qr_core_likert]))
...
#| fig-cap: "Note: Bottom-left cells show scatterplots; diagonal cells show variable distributions. Top-right values are Pearson *r* coefficients. Correlation strength: weak (|r| < 0.3), moderate (0.3 < |r| < 0.5), strong (|r| > 0.5). * p < .05, ** p < .01, *** p < .001. Plot may be simplified, or variables/smoothers removed, due to limited sample size."

df_pairs_column_names <- intersect(
  c("q1_methodology", "q2_calculation", "q3_visual_representation", 
    "q4_interpretation", "q5_evaluation", "q6_coherence_purpose", 
...
if (length(df_pairs_column_names) >= 2) {
  df_pairs_candidate <- df_numeric[, df_pairs_column_names, drop = FALSE]
  
  valid_columns_selector <- vapply(df_pairs_candidate, function(col_data) {
    num_non_na_values <- sum(!is.na(col_data))
    variance_of_non_na <- var(na.omit(col_data))
    !(!is.numeric(col_data) || num_non_na_values < 2 || is.na(variance_of_non_na) || variance_of_non_na == 0)
  }, logical(1))
  
  df_pairs <- df_pairs_candidate[, valid_columns_selector, drop = FALSE]

  if (ncol(df_pairs) >= 2 && nrow(df_pairs) > 0) {
    new_plot_names <- unlist(shortlabels[names(df_pairs)])
    if (length(new_plot_names) == ncol(df_pairs) && !any(is.na(new_plot_names))) {
      names(df_pairs) <- new_plot_names
    }

    original_par_settings <- par(mar = c(6, 5, 4, 2) + 0.1, no.readonly = TRUE)
    on.exit(par(original_par_settings))
